...
// import { getPresupuestosByEquipo } from "../api/PresupuestoApi.jsx";
// import { updateIngreso } from "../api/IngresoApi.jsx";
// import { deletePresupuesto } from "../api/PresupuestoApi";
// //componentes
// // import CargaPresupuestoModal from "../components/Presupuesto/CargaPresupuestoModal.jsx";
// // import EditarPresupuestoModal from "../components/Presupuesto/EditarPresupuestoModal.jsx";
// import CambiarEstadoModal from "../components/General/CambiarEstadoModal.jsx";
// import AlertNotification from "../components/Alerta/AlertNotification.jsx";
// import PresupuestoModal from "../components/Presupuesto/PresupuestoModal";
...
//   // Abrir modal de presupuesto en modo edicion
//   const abrirModalEditar = (presupuesto) => {
//   setPresupuestoParaEditar(presupuesto);            // 🔹 modo edición
//   setIngresoSeleccionado({ id: presupuesto.ingreso_id }); // 🔹 clave
//   setIsPresupuestoOpen(true);
// };

// const abrirModalCrear = () => {
//   if (!ingresoActual) {
//     console.warn("Ingreso aún no cargado, espera a que esté disponible.");
//     return;
//   }

//   setPresupuestoParaEditar(null);
//   setIngresoSeleccionado(ingresoActual);
//   setIsPresupuestoOpen(true);
// };
...
//     const listaPresupuestos = await getPresupuestosByEquipo(id);
//     setPresupuestos(
//       [...listaPresupuestos].sort(
//         (a, b) => new Date(b.fecha_presupuesto) - new Date(a.fecha_presupuesto)
//       )
//     );
//   };
...
//   //     try {
//   //       setLoading(true);
//   //       const response = await getEquipoById(id);
//   //       setData(response);

//   //       const listaPresupuestos = await getPresupuestosByEquipo(id);
//   //       const ordenados = [...listaPresupuestos].sort(
//   //         //CHEKEAR FUNCIIONAMIENTO DE FECHAS
//   //         (a, b) => new Date(b.fecha_presupuesto) - new Date(a.fecha_presupuesto)
//   //       );
...
//     // ⬇️ Establece el ingreso actual (será usado por el modal)
//     ingresoActual(response.detalles.ingreso);
...
// // 🔁 useEffect solo la invoca al montar el componente o cuando cambia el id
// useEffect(() => {
//   fetchData();
// }, [id]);
...
//   const ingresoActual = detalles.ingreso || null;
...
//           <p className="text-gray-500">Ingreso: {fechaIngreso}</p>
//           <p className="text-sm text-gray-400">
//             Cliente: {cliente.nombre} {cliente.apellido} | {cliente.celular}
//           </p>
//           <p className="text-sm text-gray-400">
//             Estado: {ingresoActual?.estado || "Sin estado definido"}
//           </p>
//           <p className="text-sm text-gray-400">
//             Fecha de egreso:{" "}
...
//                     <p className="text-sm text-gray-400">
//                       Fecha:{" "}
//                       {new Date(p.fecha_presupuesto).toLocaleDateString("es-AR")}
//                     </p>
//                     <p className="text-sm">Costo: ${p.costo_presupuesto}</p>
//                     <p className="text-sm">Total: ${p.total_presupuesto}</p>
//                     {p.observaciones && (
//                       <p className="text-gray-500 text-sm italic">{p.observaciones_presupuesto}</p>
//                     )}
//                     <p
//                       className={`text-sm font-medium ${p.estado_presupuesto === "aprobado"
//                           ? "text-green-400"
//                           : p.estado_presupuesto === "no aprobado"
//                             ? "text-red-400"
//                             : "text-yellow-400"
//                         }`}
//                     >
//                       Estado: {p.estado_presupuesto || "pendiente"}
//                     </p>
//                   </div>
...
//                     <button
//                       onClick={() => handleDeletePresupuesto(p.presupuesto_id)}
...
//         ingresoId={ingresoActual?.ingreso_id}
//         estadoActual={ingresoActual?.estado}
//         fechaIngresoActual={ingresoActual?.fecha_ingreso}
//         fechaEgresoActual={ingresoActual?.fecha_egreso}
//         onSuccess={(msg) => setAlert({ type: "success", message: msg })}
//         onError={(msg) => setAlert({ type: "error", message: msg })}
//         onUpdated={async () => {
//           await fetchData();
//           await refrescarPresupuestos();
//           showAlert("Presupuesto creado correctamente", "success");
//         }} // 🔹 Refresca los datos
//       />
//       {/* Modal presupuesto */}
...
//             ingresoSeleccionado={ingresoActual}
//             onPresupuestoCreado={async () => {
//               await refrescarPresupuestos();
//               showAlert("Presupuesto creado correctamente", "success");
//             }}
//       /> */}
...
//         isOpen={isEditarPresupuestoOpen}
//         onClose={cerrarModalEditar}
//         presupuesto={presupuestoSeleccionado}
//         onPresupuestoActualizado={async () => {
//           await refrescarPresupuestos();
//           cerrarModalEditar();
//           showAlert("Presupuesto actualizado correctamente", "success");
//         }}
...
//         esEdicion={!!presupuestoParaEditar}
//         onPresupuestoGuardado={fetchData}
//       />
...
// import EditarPresupuestoModal from "../components/EditarPresupuestoModal.jsx";
import CambiarEstadoModal from "../components/General/CambiarEstadoModal.jsx";
import AlertNotification from "../components/Alerta/AlertNotification.jsx";
...
const DetalleEquiposPage = () => {
  ...
  const [isPresupuestoOpen, setIsPresupuestoOpen] = useState(false);
  const [isEstadoModalOpen, setIsEstadoModalOpen] = useState(false);
  const [isEditarPresupuestoOpen, setIsEditarPresupuestoOpen] = useState(false);
  ...
  // const handleUpdateIngreso = async (ingresoActual) => {
  //   console.log(ingresoActual)
  //   if (!ingresoActual?.id) {
  //     showAlert("No hay ingreso activo para actualizar", "error");
  //     return;
  //   }
  //   try {
  //     await updateIngreso(ingresoActual.id, { ...ingresoActual, estado, fecha_egreso });
  //     showAlert("Estado actualizado correctamente", "success");
  //     setIsEstadoModalOpen(false);
  //     // recargar todo (equipo + presupuestos)
  //     await refrescarPresupuestos();
  //   } catch (err) {
      ...
      <div className="flex-1 p-6 overflow-y-auto space-y-6">
        ...
        <div className="bg-neutral-800 p-4 rounded shadow max-h-64 overflow-y-auto">
          ...
          {presupuestos.length > 0 ? (
            <ul className="space-y-3">
              {presupuestos.map((p) => (
                <li key={p.presupuesto_id ?? p.id ?? `${p.ingreso_id}-${p.fecha_presupuesto}`}
                  className="border-b border-neutral-700 pb-2 last:border-0 flex justify-between items-start">
                  <div>
                    <p className="text-sm text-gray-400">
                      Fecha: {new Date(p.fecha_presupuesto || p.fecha || "").toLocaleDateString("es-AR")}
                    </p>
                    <p className="text-sm">Costo: ${p.costo_presupuesto ?? p.costo}</p>
                    <p className="text-sm">Total: ${p.total_presupuesto ?? p.total}</p>
                    <p className="text-sm">Observaciones: {p.observaciones_presupuesto ?? p.observaciones}</p>
                    <p className={`text-sm font-medium ${(p.estado_presupuesto || p.estado) === "aprobado" ? "text-green-400"
                        : (p.estado_presupuesto || p.estado) === "no aprobado" ? "text-red-400"
                          : "text-yellow-400"
                      }`}>
                      Estado: {p.estado_presupuesto ?? p.estado ?? "pendiente"}
                    </p>
                    {p.observaciones && <p className="text-gray-500 text-sm italic">{p.observaciones}</p>}
                  </div>

                  {/* Modificar presupuesto */}
                  <div className="flex flex-col gap-2">
                    <button onClick={() => handleEditarPresupuesto(p)}
                      className="ml-4 bg-yellow-600 hover:bg-yellow-700 px-3 py-1 rounded text-sm font-medium self-start">
                      Modificar
                    </button>
                    <button
                      onClick={() => handleEliminarPresupuesto(p.presupuesto_id)}
                      className="ml-4 bg-red-600 hover:bg-red-700 px-3 py-1 rounded text-sm font-medium self-start"
                    >
                      Eliminar
                    </button>
                  </div>
