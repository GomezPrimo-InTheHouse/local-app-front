...
const VentasPage = () => {
  ...
  const handleGuardarVenta = async (ventaPayload) => {
    try {
      if (editingVenta) {
        await updateVenta(editingVenta.venta_id, ventaPayload);
        Swal.fire({
          title: '¡Actualizada!',
  ...
  const handleEditVenta = (venta) => {
    // 🔹 Ya no necesitamos una llamada a getVentaById, ya que la lista de ventas ya trae los detalles
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-screen bg-neutral-900 text-white overflow-hidden">
      <aside className="w-full lg:w-1/4 bg-neutral-800 p-6 hidden lg:flex flex-col justify-start shadow-xl">
        <button
          onClick={() => navigate("/")}
