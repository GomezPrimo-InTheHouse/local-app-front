...
const EquipoModal = ({ isOpen, onClose, onSubmit, equipoSeleccionado }) => {
  ...
  useEffect(() => {
    ...
    if (equipoSeleccionado) {
      setFormData({
        ...
        imei:       equipoSeleccionado.imei       || "",
        patron:     equipoSeleccionado.ultimo_patron  || "",
        password:   equipoSeleccionado.ultima_password || "",
      });
    } else {
      setFormData({ tipo: "", marca: "", modelo: "", cliente_id: "", estado_id: "", imei: "", patron: "", password: "" });
    }
  }, [isOpen, equipoSeleccionado]);
  ...
  useEffect(() => {
    ...
    if (clienteSel) {
      ...
      setSearch(`${equipoSeleccionado.cliente_nombre || ""} ${equipoSeleccionado.cliente_apellido || ""}`.trim());
    }
  }, [equipoSeleccionado?.cliente_id, clientes]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.cliente_id) { alert("Debe seleccionar un cliente."); return; }
    if (!formData.estado_id)  { alert("Debe seleccionar un estado."); return; }

    // 1. Actualizar el equipo (sin password ni patron — no existen en la tabla equipo)
    await onSubmit({
      tipo:       formData.tipo,
    ...
    // 2. Si tiene OT activa, actualizar password y patron en la OT
    const otId = equipoSeleccionado?.ultima_ot_id;
    if (otId && (formData.password !== undefined || formData.patron !== undefined)) {
      try {
        ...
        console.error("Error actualizando contraseña en OT:", err);
      }
    }
      ...
      <div className="bg-neutral-800 rounded-xl w-full max-w-lg shadow-2xl text-neutral-100 flex flex-col max-h-[95vh] sm:max-h-[90vh]">
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4 [scrollbar-width:thin]">
          <form onSubmit={handleSubmit} className="space-y-4" id="form-equipo">
            ...
            {/* Separador OT */}
            {equipoSeleccionado?.ultima_ot_id && (
              <>
                <div className="flex items-center gap-2 pt-1">
                  <div className="h-px flex-1 bg-white/10" />
                  <span className="text-xs text-neutral-500">Datos de la última OT</span>
                  <div className="h-px flex-1 bg-white/10" />
                </div>

                {/* Contraseña / PIN — alfanumérico */}
                <div>
                  <label className="block text-sm font-medium text-neutral-300 mb-1">
