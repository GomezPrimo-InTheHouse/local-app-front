...
const getEstadoStyles = (nombre) => {
  const s = (nombre || "").toLowerCase();
  if (s.includes("finaliz"))
    return { border: "border-emerald-700/60", badge: "bg-emerald-600/20 text-emerald-400", glow: "hover:shadow-emerald-900/30" };
  if (s.includes("reparac") || s.includes("diagnos"))
    return { border: "border-blue-700/60", badge: "bg-blue-600/20 text-blue-400", glow: "hover:shadow-blue-900/30" };
  if (s.includes("reingres") || s.includes("ingresad"))
    return { border: "border-amber-700/60", badge: "bg-amber-600/20 text-amber-400", glow: "hover:shadow-amber-900/30" };
  if (s.includes("entregad") || s.includes("cobrad"))
    return { border: "border-purple-700/60", badge: "bg-purple-600/20 text-purple-400", glow: "hover:shadow-purple-900/30" };
  if (s.includes("rechaz") || s.includes("abandon"))
    return { border: "border-red-700/60", badge: "bg-red-600/20 text-red-400", glow: "hover:shadow-red-900/30" };
  return { border: "border-neutral-700/60", badge: "bg-neutral-700/40 text-neutral-300", glow: "hover:shadow-purple-900/20" };
};
...
const formatDateShort = (s) => {
  if (!s) return "N/A";
  const d = new Date(s);
  return isNaN(d.getTime()) ? "N/A" : d.toLocaleDateString("es-AR", { year: "numeric", month: "2-digit", day: "2-digit" });
};
...
const IconAlert   = () => (<svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01M6.832 20h10.336A2.973 2.973 0 0021 17.027V6.973A2.973 2.973 0 0017.168 4H6.832A2.973 2.973 0 003 6.973v10.054A2.973 2.973 0 006.832 20z" /></svg>);
const IconKey     = () => (<svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" /></svg>);
const IconDoc     = () => (<svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>);
const IconPlus    = () => (<svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>);
...
const EquipoCard = ({
  ...
  onDelete,
  onNuevaOT,
  onNuevoPresupuesto, // (eq) => void — abre PresupuestoModal con ultima_ot_id
}) => {
  const styles        = getEstadoStyles(estadoNombre);
  const clienteNombre = `${eq.cliente_nombre || "Anónimo"} ${eq.cliente_apellido || ""}`.trim();
  const costoTotal    = balance?.costo_total   ?? 0;
  ...
  // Si tiene OT activa → mostrar botón presupuesto
  // Si no tiene OT → mostrar botón Nueva OT
  const tieneOT = !!eq.ultima_ot_id;
    ...
    `}>
      ...
      <div className="flex justify-between items-start gap-2">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="text-xl leading-none">{getIconoTipo(eq.tipo)}</span>
            <p className="text-base font-bold text-white truncate">{eq.marca} {eq.modelo}</p>
          </div>
          <p className="text-[11px] text-neutral-500 uppercase tracking-wide">
            {eq.tipo} · ID {eq.id}{eq.imei && ` · IMEI ${eq.imei}`}
          </p>
        </div>
        <div className="flex flex-col items-end gap-1 flex-shrink-0">
          <span className={`px-2 py-0.5 text-[11px] font-semibold rounded-full whitespace-nowrap ${styles.badge}`}>
      ...
      <div className="grid grid-cols-2 gap-1.5">
        ...
        {/* Botón principal — presupuesto si tiene OT, nueva OT si no */}
        {tieneOT ? (
          <button
            onClick={() => onNuevoPresupuesto(eq)}
            className="col-span-2 flex items-center justify-center gap-1.5 px-2 py-2 rounded-lg bg-emerald-700/30 hover:bg-emerald-700/50 text-emerald-300 text-xs font-semibold transition-colors border border-emerald-700/30"
          >
            <IconDoc /> + Nuevo Presupuesto
          </button>
        ) : (
          <button
            onClick={() => onNuevaOT(eq)}
            className="col-span-2 flex items-center justify-center gap-1.5 px-2 py-2 rounded-lg bg-amber-700/30 hover:bg-amber-700/50 text-amber-300 text-xs font-semibold transition-colors border border-amber-700/30"
          >
            <IconPlus /> Nueva Orden de Trabajo
          </button>
        )}
