...
const parseNumber          = (v) => parseInt(String(v).replace(/\D/g, ''), 10) || 0;
const formatNumberWithDots = (n) => n === 0 ? '0' : Number(n).toLocaleString("es-AR", { minimumFractionDigits: 0, maximumFractionDigits: 0 });
const formatPrice          = (p) => Number(p || 0).toLocaleString("es-AR", { minimumFractionDigits: 0, maximumFractionDigits: 0 });
const formatDateShort      = (s) => {
  if (!s) return "N/A";
  const d = new Date(s);
  return isNaN(d.getTime()) ? "N/A" : d.toLocaleDateString("es-AR");
};
...
const HistorialPagosModal = ({ isOpen, onClose, clienteId, clienteNombre }) => {
  ...
  // ── Cálculo consolidado — solo presupuestos Aprobados ──
  const consolidatedData = useMemo(() => {
    let totalAdeudado          = 0;
    let todosPagos             = [];
    let presupuestosPendientes = [];
    ...
    equiposHistorial.forEach(eq => {
      const presupuesto    = eq.presupuesto_final;
      // ⬇️ CAMBIO: orden_trabajo_id en lugar de ingreso_id
      const ordenTrabajoId = eq.orden_trabajo_id;

      if (!presupuesto || !ordenTrabajoId) return;

      const tieneAprobado = presupuesto.presupuestos?.some(p => p.estado_id === ESTADO_APROBADO);
      if (!tieneAprobado) return;

      if (presupuesto.saldo_pendiente > 0) {
        totalAdeudado += presupuesto.saldo_pendiente;
        presupuestosPendientes.push({
          orden_trabajo_id: ordenTrabajoId,   // ⬇️ CAMBIO
          presupuesto_id:   presupuesto.id,
          saldo_pendiente:  presupuesto.saldo_pendiente,
          fecha_ingreso:    eq.fecha_ingreso,
        });
    ...
    return {
      totalAdeudado,
      historialPagosUnificado: todosPagos.sort((a, b) => new Date(b.fecha_pago) - new Date(a.fecha_pago)),
  ...
  useEffect(() => {
    if (isOpen && clienteId) fetchHistorial();
  }, [isOpen, clienteId, fetchHistorial]);
  ...
  const handleNuevoPago = async (e) => {
    ...
    if (montoNumerico > 0 && montoNumerico > totalAdeudado && totalAdeudado > 0) {
      Swal.fire('Error', `El pago no puede exceder el saldo pendiente ($${formatPrice(totalAdeudado)}).`, 'warning');
      return;
    }
    ...
    try {
      // Pago positivo: distribuir entre OTs pendientes
      for (const p of presupuestosPendientes) {
        if (montoRestante <= 0) break;
        const montoAPagar = Math.min(montoRestante, p.saldo_pendiente);
        if (montoAPagar > 0) {
          // ⬇️ CAMBIO CLAVE: orden_trabajo_id en lugar de ingreso_id
          await createPagoAbono({
          ...
          montoRestante -= montoAPagar;
          pagosRealizados.push(p);
        }
      }

      // Pago negativo (ajuste/reverso)
      if (montoNumerico < 0) {
        ...
        const primerOT = presupuestosPendientes[0];
        // ⬇️ CAMBIO CLAVE: orden_trabajo_id en lugar de ingreso_id
        await createPagoAbono({
          orden_trabajo_id: primerOT.orden_trabajo_id,
          monto:            montoNumerico,
          observaciones,
          metodo_pago,
        });
        pagosRealizados.push(primerOT);
      }

      const message = montoNumerico > 0
        ? `✅ Abono de $${formatPrice(montoNumerico)} registrado en ${pagosRealizados.length} orden${pagosRealizados.length !== 1 ? 'es' : ''}.`
        : `✅ Ajuste/Reverso de $${formatPrice(montoNumerico)} registrado.`;

      Swal.fire({
        title: 'Éxito', text: message, icon: 'success',
        timer: 3000, showConfirmButton: false,
        customClass: { popup: "bg-neutral-800 text-white border border-neutral-700 rounded-lg shadow-xl" },
      });
      ...
      Swal.fire({
        title: 'Error', text: "❌ Error al registrar el pago.", icon: 'error',
        customClass: { popup: "bg-neutral-800 text-white border border-neutral-700 rounded-lg shadow-xl" },
      });
    } finally {
      if (success) {
    ...
    >
      <div className="bg-neutral-900 border border-neutral-700 rounded-xl shadow-2xl max-w-5xl w-full max-h-[90vh] flex flex-col">
        ...
        <div className="p-5 border-b border-neutral-800 bg-neutral-800/50 relative">
          ...
          <p className="text-sm text-neutral-400">{equiposHistorial.length} equipos en historial.</p>
          <button onClick={onClose} className="absolute top-4 right-4 text-neutral-500 hover:text-white transition text-3xl leading-none">&times;</button>
        </div>

        {/* Contenido */}
        <div className="flex-grow grid md:grid-cols-3 gap-4 p-5 overflow-hidden">

          {/* COLUMNA 1: REGISTRO DE PAGO + HISTORIAL */}
          <div className="md:col-span-1 border-r border-neutral-800 pr-4 flex flex-col overflow-y-auto space-y-4">
            <h3 className="text-lg font-semibold text-white flex items-center gap-2">
              <IconDollar /> Registro de Abono Consolidado
            </h3>
            ...
            <div className={`p-4 rounded-xl shadow-lg border ${totalAdeudado > 0 ? 'bg-red-900/40 border-red-500/50' : 'bg-emerald-900/40 border-emerald-500/50'}`}>
              ...
              <small className="text-neutral-500 block mt-1">Suma de saldos pendientes de presupuestos aprobados.</small>
            </div>
            ...
            <form onSubmit={handleNuevoPago} className="p-3 bg-neutral-800 border border-neutral-700 rounded-lg grid grid-cols-4 gap-3">
              ...
              <div className="col-span-2 relative">
                ...
                <input type="text" name="monto" placeholder="Monto"
                  className="w-full pl-7 pr-3 py-2 rounded bg-neutral-900 border border-neutral-700 text-white placeholder-neutral-500 text-sm focus:border-purple-500"
                  required value={pagoMonto} onChange={handleMontoChange} />
              </div>
            ...
            <div className="pt-4 border-t border-neutral-800">
              <h4 className="text-sm font-semibold text-neutral-400 flex items-center gap-1 mb-2">
                <IconHistory /> Historial de Abonos:
              </h4>
              {historialPagosUnificado.length === 0 ? (
                  ...
                  {historialPagosUnificado.map(pago => (
                    <li key={pago.pago_id} className="py-2 flex justify-between items-start text-sm">
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-neutral-500">{pago.metodo_pago} ({pago.equipo || 'N/A'})</span>
                      </div>
                      <div className="flex flex-col items-end">
                        <span className="text-xs text-neutral-400 italic max-w-[50%] overflow-hidden text-right whitespace-nowrap text-ellipsis" title={pago.observaciones}>
                          {pago.observaciones}
          ...
          <div className="md:col-span-2 flex flex-col space-y-4 overflow-y-auto pl-4">
            ...
            {loading ? (
              ...
              <p className="text-neutral-500 text-lg md:pt-10 text-center">No hay equipos en historial.</p>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {equiposHistorial.map(eq => {
                  const presupuesto   = eq.presupuesto_final;
                  ...
                  return (
                    <div key={eq.equipo_id}
                      ...
                      <p className="font-bold text-sm text-purple-300 line-clamp-1">{eq.marca} {eq.modelo}</p>

                      {tieneAprobado && totalVenta > 0 ? (
                        <div className="space-y-1">
                          <div className="flex justify-between text-xs text-neutral-400">
                            <span>Costo repuestos:</span>
                            <span className="font-semibold text-neutral-300">${formatPrice(costoTotal)}</span>
                          </div>
                          <div className="flex justify-between text-xs text-neutral-400">
                            <span>Total (c/ MO):</span>
