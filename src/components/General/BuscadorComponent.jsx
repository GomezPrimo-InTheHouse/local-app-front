...
import { getEquipos } from "../../api/EquiposApi.jsx";
...
const BuscadorComponent = ({ onBuscar }) => {
  ...
  // ✅ Filtrar clientes por nombre o celular (null-safe)
  useEffect(() => {
    const filtro = (search ?? "").trim().toLowerCase();
    ...
    const filtrados = (clientes ?? []).filter((cli) => {
      ...
      const celular = (cli?.celular ?? "").toString().toLowerCase();
      const apellido = (cli?.apellido ?? "").toString().toLowerCase();

      // Si querés que también matchee por apellido, ya está incluido
      return (
